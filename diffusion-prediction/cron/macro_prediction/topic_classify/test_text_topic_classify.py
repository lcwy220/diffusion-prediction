...
def input_data():#测试输入
    ...
    for mid,w_text in reader:
        ...
        word_list = dict()
	for word in words:
            if (word[1] in cx_dict) and 3 < len(word[0]) < 30 and (word[0] not in black_word) and (word[0] not in single_word_whitelist):#choose n. / v. / adj. from the segment results,and delete single word
