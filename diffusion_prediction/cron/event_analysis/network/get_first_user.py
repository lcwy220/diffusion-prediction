...
def get_first_node(topic, start_ts, end_ts, windowsize, date):
    ...
    if topic and topic != '':
        ...
        #print 'es_search_weibos::::::;',es_search_weibos
        if not time_top_nodes:
            ...
            uid_package = []
            print 'type(time_top_nodes)::::::',type(time_top_nodes)
            for node in time_top_nodes:
                #first_node_results[node] = []
                # print 'node:', node
                node = node['_source']
                uid = node['uid']
                #print uid
                ...
                first_node_results[uid] = []

                first_node_results[uid].append([topic, date, windowsize, uid, timestamp, user_info, user_weibos])
                #save_first_nodes_es(topic, date, windowsize, uid, timestamp, user_info, user_weibos)
                #print user_info

        return first_node_results
...
def save_first_nodes_es(topic, date, windowsize, uid, timestamp, user_info, weibo_info, user_domain = 'other'):
