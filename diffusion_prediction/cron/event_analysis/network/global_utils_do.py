...
def load_train():
    ...
    for i in txt_labels:
        ...
        reader = csv.reader(file(BASE_DIR+'/network/domain/topic_dict/%s.csv'% i, 'rb'))
        word_dict = dict()
        count = 0
...
def getAdminWords():
    ...
    f = open(BASE_DIR+'/network/domain/domain_dict/adw.txt', 'r')
...
def getMediaWords():
    ...
    mediaf = open(BASE_DIR+'/network/domain/domain_dict/mediaw.txt','r')
...
def getBusinessWords():
    ...
    f = open(BASE_DIR+'/network/domain/domain_dict/businessw.txt', 'r')
...
def readProtoUser():
    ...
    f = open(BASE_DIR+"/network/domain/protou_combine/protou.txt", "r")
    protou = dict()
    for line in f:
        area=line.split(":")[0]
...
def readTrainUser():
    ...
    for i in range(0,len(txt_list)):
        ...
        f = open(BASE_DIR+"/network/domain/domain_combine/%s.txt" % txt_list[i],"r")
        item = []
        for line in f:
            line = line.strip('\r\n')
            item.append(line)
