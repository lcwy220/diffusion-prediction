...
sys.path.append('../../../')
from global_config import weibo_es,index_event_network_topic_identification,type_event_network_topic_identification
from global_config import index_event_analysis_results,type_event_analysis_results

def SnowballSampling_es(di_g, g, topic, network_type,pr_data):
    ...
    if network_type==1:
        ...
        items_pr = weibo_es.search(index=index_name,doc_type=index_type,body=query_body)['hits']['hits']
        '''
        items_pr = {}
        print 'type::::pr_data',type(pr_data)
        for uid ,rank in pr_data.iteritems():
            if not rank > 4000:
                items_pr[uid] = rank

    #di_centerids = [str(item.userId) for item in items_pr if di_g.has_node(str(item.userId))]
    di_centerids = [str(uid) for uid,rank in items_pr.iteritems() if di_g.has_node(str(uid))]
    ...
    centerids = [str(uid) for uid,rank in items_pr.iteritems() if g.has_node(str(uid))]
    print centerids[:3]
    sample_g = nx.Graph()
    taboo_set = set()
