...
from utils import acquire_topic_name, acquire_topic_id, \
        save_rank_results_es, save_gexf_results, save_attribute_dict, \
...
from global_config import weibo_es,index_event_analysis_results,type_event_analysis_results
...
def compute_network(topic, start_ts, end_ts):
    ...
    if True:
        ...
        first_node_results = get_first_node(topic_pinyin_name, start_ts, end_ts, windowsize, date)
        ...
        all_uid_pr,data_dict,sorted_uids = pagerank_rank(TOPK, date, windowsize, topic_pinyin_name)
        ...
        #print gexf
        #print '*************************'*10
        long_gexf = save_gexf_results(topic_pinyin_name, date, windowsize, gexf, gexf_type)

        network_results['long_gexf'] = long_gexf

        print 'start fu_tr'
        maker_results, pusher_results = get_interval_count(topic_pinyin_name, date, windowsize)
        ...
        try:
            tem_exist = weibo_es.get(index=index_name,doc_type=index_type,id=id)['_source']
            weibo_es.update(index=index_name,doc_type=index_type,id=id,body={'doc':{'network_results':network_results}})
        except Exception,e:
            weibo_es.index(index=index_name,doc_type=index_type,id=id,body={'network_results':network_results})
