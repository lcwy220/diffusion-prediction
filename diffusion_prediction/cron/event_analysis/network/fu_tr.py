...
def get_interval_count(topic, date, windowsize):
    ...
    pusher_results = save_trend_pusher_es(topic, date, windowsize, trend_pusher)
...
def save_trend_maker_es(topic, date, windowsize, trend_maker):
    ...
    for maker in makers:
        ...
        # 修改model
        '''
        mappings_event_network_trend_maker()
        index_name = index_event_network_trend_maker
        index_type = type_event_network_trend_maker
        '''
        item = {}
        ...
        item['rank'] = rank
        item['value'] = value
        item['key_item'] = json.dumps(key_item)

        maker_results[uid]= item
...
def save_trend_pusher_es(topic, date, windowsize, trend_pusher):
    #xapian_search_weibo = getXapianWeiboByTopic(topic_xapian_id) # topic id 要做一下处理
    pusher_results = {}
    ...
    rank = 0
    user_exist_list = []
    #下面一段查询作用不大（查到不为空时，将其删掉），因为下面会对其更新。
    '''
    ...
    for pusher in pushers:
        ...
        timestamp = int(weibo_info[0]['_source']['timestamp'])
        '''
        mappings_event_network_trend_pusher()
        index_name = index_event_network_trend_pusher
        index_type = type_event_network_trend_pusher
        '''
        item = {}
