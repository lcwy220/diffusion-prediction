...
from snowball1 import SnowballSampling_es
...
from utils import save_rank_results_es, save_ds_rank_results, acquire_topic_name, \
...
def pagerank_rank(top_n, date, window_size, topicname):
    ...
    data = save_rank_results_es(sorted_uids, 'topic', 'spark_pagerank', date, window_size, topicname, all_uid_pr)
    #ds_data = save_ds_rank_results(ds_sorted_uids, 'topic', 'spark_pagerank', date, window_size, topicname, ds_all_uid_pr)

    return all_uid_pr, data, sorted_uids
    #return all_uid_pr
...
def make_network_graph(current_date, topic_pinyin_name, window_size, all_uid_pr, pr_data,sorted_uids,new_attribute_dict, key_user_labeled=True):
    date = current_date
    '''
    ...
    #1227 jln for 
        ...
        new_G, new_gg = SnowballSampling_es(G, gg, topic_pinyin_name, network_type,pr_data)
