...
'''
sys.path.append('../../../')
from global_config import index_event_network_topic_identification,type_event_network_topic_identification
...
def save_rank_results_es(sorted_uids, identifyRange, method, date, window, topicname, all_uid_pr):
    
    #存放源头转发网络pagerank的计算结果
    #mappings_event_network_topic_identification()
    index_name = index_event_network_topic_identification
    index_type = type_event_network_topic_identification
    ...
    for uid in sorted_uids:
        ...
        if identifyRange == 'topic':
            item['en_name'] = topicname
            item['rank'] = rank
            item['uid'] = uid
            item['identifyDate'] = date
            item['identifyWindow'] = window
            item['identifyMethod'] = method
            item['pr'] = pr
...
def save_gexf_results(topic, identifyDate, identifyWindow, identifyGexf, gexf_type):
    ...
    value = identifyGexf
    #gexf2es(key, value)
    long_gexf = save_long_gexf(topic, identifyDate, identifyWindow, identifyGexf)
