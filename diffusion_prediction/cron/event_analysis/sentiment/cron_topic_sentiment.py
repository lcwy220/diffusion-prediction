...
def save_rt_results_es(calc, topic, results, during, klimit=TOP_KEYWORDS_LIMIT, wlimit=TOP_WEIBOS_LIMIT):
    ...
    elif calc == 'weibos':    #{'时间戳'：{'情绪1'：[{微博字段},{微博字段}],'情绪2'：[]}}

        #mappings_event_sentiment_weibo()
        #index_name = index_event_sentiment_weibo
        #index_type = type_event_sentiment_weibo

        item = {}
        for time,sen_dict in results.iteritems():
            id = topic + '_' + time
            for sentiment, weibo in sen_dict.iteritems():
                item['en_name'] = topic
                item['end_ts'] = time
                item['range'] = during
                item['sentiment'] = sentiment
    ...
    elif calc == 'geo_count':   # 地理位置 #{'sentiment':[shijian,{['province':('provice':cishu),()],'city':[(city:cishu)}]}
        #print results
        #mappings_event_sentiment_geo()
        #index_name = index_event_sentiment_geo
        #index_type = type_event_sentiment_geo

        item = {}

        for sentiment, geo in results.items():
            end_ts = geo[0]
            geo_count = geo[1]
            item['en_name'] = topic
            item['end_ts'] = end_ts
    ...
    elif calc == 'sentiment_results':

        id = topic

        try:
            item_exist = weibo_es.get(index=index_name,doc_type=index_type,id=id)['_source']
            weibo_es.update(index=index_name,doc_type=index_type,id=id,body={'doc':{'sentiment_results':results}})
        except Exception,e:
            weibo_es.index(index=index_name,doc_type=index_type,id=id,body={'sentiment_results':results})
...
def sentimentTopic(topic,start_ts, over_ts, sort_field=SORT_FIELD, save_fields=RESP_ITER_KEYS, \
    ...
    if topic and topic != '':
        ...
        for i in range(interval, 0, -1):    #时间段取每900秒的
            ...
            sentiment_results['geo_count'][end_ts] = emotions_geo_count
