...
get_maker_weibos_byts_es, get_pusher_weibos_byts, get_pusher_weibos_byhot,\
get_maker_weibos_byhot, gexf_process,get_top_pagerank
import json
...
def GetPusher():
    ...
    date = ts2datetime(end_ts)
    windowsize = (end_ts - start_ts) / Day # 确定时间跨度的大小
    results = get_trend_pusher_es(topic, date, windowsize)
    return json.dumps(results)
...
def GetMaker():
    ...
    date = ts2datetime(end_ts)
    windowsize = (end_ts - start_ts) / Day # 确定时间跨度的大小
    results = get_trend_maker_es(topic, date, windowsize)
    return json.dumps(results)
...
def GetPusherWeibosByts():
    ...
    results = get_pusher_weibos_sort_es(topic, date, windowsize,sort_item)
    return json.dumps(results)
...
def GetMakerWeibosByts():
    ...
    windowsize = (end_ts - start_ts) / Day # 确定时间跨度的大小
    sort_item = request.args.get('sort_item','timestamp')
    results = get_maker_weibos_sort_es(topic, date, windowsize,sort_item)
    return json.dumps(results)
