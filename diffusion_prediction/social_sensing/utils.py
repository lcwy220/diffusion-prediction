...
index_manage_sensing_task = "manage_sensing_task"
task_doc_type = "task"
...
# 获得一段时间内的文本，按序排列
def get_text_detail(ts, text_type, order, size=100):
    results = []

    #print social_sensors
    if int(text_type) == 0: # 热门原创微博
        results = get_origin_weibo_detail(ts, size, order, 1)

    elif int(text_type) == 1: # 热门转发微博
        results = get_origin_weibo_detail(ts, size, order, 2)


    elif int(text_type) == 7: # 敏感微博
        results = get_origin_weibo_detail(ts, size, order, 3)
