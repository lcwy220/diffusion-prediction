...
import math
from keyword_extraction import get_weibo
from cp_time_utils import ts2HourlyTime,datetime2ts,full_datetime2ts,ts2datetime
reload(sys)
sys.path.append('../../')
...
def get_symbol_weibo(topic,start_ts,end_ts,unit=MinInterval):  #鱼骨图
    ...
    for clusterid,contents in symbol_weibos.iteritems():
        ...
        for i in contents:
            ts = full_datetime2ts(i['datetime'])
            print 're::::::',re.findall(r'【.*】',i['content'].encode('utf8'))
...
if __name__ == '__main__':
    ...
    #opinion=['姐姐', '综艺节目', '网络']
    opinion=[u'毛泽东', u'纪念日', u'亲人', u'毛泽东思想', u'万岁']
    #get_weibo_content('aoyunhui',1468944000,1471622400,opinion)
    #get_during_keywords('aoyunhui',1468944000,1469707652)
    #print get_during_keywords('mao_ze_dong_dan_chen_ji_nian_ri',1482681600,1483113600)
    #print get_topics_river('mao_ze_dong_dan_chen_ji_nian_ri',1482681600,1483113600,unit=MinInterval)
    #print get_symbol_weibo('mao_ze_dong_dan_chen_ji_nian_ri',1482681600,1483113600,unit=MinInterval)
    #print get_subopinion('mao_ze_dong_dan_chen_ji_nian_ri')

    print get_weibo_content('mao_ze_dong_dan_chen_ji_nian_ri',1482681600,1483113600,opinion,sort_item='timestamp')